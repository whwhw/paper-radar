...
Uses JSON mode (response_format={"type": "json_object"}) for universal provider
compatibility (DeepSeek, OpenAI, Gemini, OpenRouter, Ollama). Response is
parsed manually with Summary.model_validate_json().
"""
...
_SCHEMA_HINT = json.dumps(Summary.model_json_schema(), ensure_ascii=False, indent=2)
...
2. **通俗解读 (plain_explanation)**：200 字以内中文，包含"背景→方法→发现→意义"四块，
   ...
   - 黑话 → 翻译成日常词
   - 假设读者是高中生
3. **关键方法 (key_method)**：可选。如果方法本身有启发，简化讲；纯工程实现就留 null
...
必须返回严格符合下面 JSON Schema 的 JSON 对象，不要包含 markdown 代码块包装、解释文字或额外字段。
